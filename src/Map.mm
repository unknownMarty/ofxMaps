...
void Map::update() {
...
#if IMAGE_POLYLINES != 1 || IMAGE_IMAGE == 1
			if (images.count(coord) == 0) {
#else
            if (imagePolyLines.count(coord) == 0) {
#endif
...
#else
                    if (imagePolyLines.count(zoomed) == 0) {
#endif					// force load of parent tiles we don't already have
						grabTile(zoomed);
					}
				}
	...
	if (recentImages.size() > numToKeep) {
        ...
        std::map<Coordinate,mapImage>::iterator iter = images.begin();
        std::map<Coordinate,mapImage>::iterator endIter = images.end();
...
void Map::touchMoved(ofTouchEventArgs &touch)
{
    ...
    if (touch.numTouches == 1)
    {
        ...
        panBy(-ofPoint((beginTouch.x-touch.x),(beginTouch.y-touch.y)));
    }
    else if (touch.numTouches == 2)
    {
		...
		ofVec2f endCenter = p0.getInterpolated( p1,0.3);
		ofVec2f startCenter = p2.getInterpolated( p3,0.3);
...
void Map::touchUp(ofTouchEventArgs &touch)
{
    mPrevTouches.clear();
    bIsZooming = false;
}
void Map::touchDoubleTap(ofTouchEventArgs &touch)
{
    rotation = 0;
}

void Map::touchCancelled(ofTouchEventArgs &touch)
{
    bIsZooming = false;
}
