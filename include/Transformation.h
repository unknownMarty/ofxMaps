...
public:
	...
	ofVec2f transform(const ofVec2f &point) {
		return ofVec2f(ax*point.x + bx*point.y + cx, ay*point.x + by*point.y + cy);
	}
	
	ofVec2f untransform(const ofVec2f &point){
		return ofVec2f((point.x*by - point.y*bx - cx*by + cy*bx) / (ax*by - ay*bx), (point.x*ay - point.y*ax - cx*ay + cy*ax) / (bx*ay - by*ax));
	}
    ...
    static Transformation deriveTransformation( const double &a1x, const double &a1y, 
        ...
        const ofVec3f d = linearSolution( a1x, a1y, a2x,
                                        b1x, b1y, b2x,
                                        c1x, c1y, c2x );
        const ofVec3f e = linearSolution( a1x, a1y, a2y,
                                        b1x, b1y, b2y,
                                        c1x, c1y, c2y );
    ...
    static ofVec3f linearSolution( const double &r1, const double &s1, const double &t1, 
