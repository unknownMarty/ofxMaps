...
public:
    ...
    ofVec2f lerp(ofVec2f a, ofVec2f b, float percent){
	...
	void setTarget(const Location &l1, const double &z1, const double &_t0) {
		...
		c0 = ofVec2f(start.column, start.row);
        c1 = ofVec2f(end.column, end.row);
	...
	double visibleWorld() {
		// how much world can we see at zoom 0?
		Coordinate topLeft = map->pointCoordinate(ofVec2f::zero()).zoomTo(0);
		Coordinate bottomRight = map->pointCoordinate(map->getSize()).zoomTo(0);
		return max(bottomRight.column-topLeft.column, bottomRight.row-topLeft.row);
	}
	...
	void update(const double &t1) {
		...
		double us = u(s);
        //ofLerp((float)c0,(float)c1,(float)(us-u0)/(u1-u0));
		ofVec2f pos = lerp(c0,c1,(us-u0)/(u1-u0));
